...
from recbole.data.noise import feast, add_noise  # 导入噪声注入和长尾分布实验函数
...
def data_preparation(config, dataset):
    ...
    if dataloaders is not None:
        ...
        if config['enable_noise_injection']:
            ...
            valid_dataset, test_dataset = add_noise(
                valid_dataset, test_dataset, dataset, noise_ratio=noise_ratio
            )
        
        # 选项2: 长尾分布实验（只保留头部/尾部物品）
        elif config['enable_long_tail_split']:
            long_tail_threshold = config['long_tail_threshold']
            valid_dataset, test_dataset = feast(
                valid_dataset, test_dataset, dataset, k=long_tail_threshold
            )
